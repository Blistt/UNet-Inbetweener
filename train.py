    ...
    for epoch in range(n_epochs):
        ...
        if epoch % display_step == 0:
            ...
            if test_dataset is not None:
                ...
                test_loss = test(test_dataset, model, criterion, epoch, batch_size=batch_size, device=device, experiment_dir=experiment_dir, display_step=display_step)
...
def test(dataset, model, criterion, epoch, batch_size=8, device='cuda:1', experiment_dir='exp/', display_step=10):
    ...
    for input1, labels, input2 in tqdm.tqdm(dataloader):
        ...
        if cur_step % display_step == 0:
            # Visualizes predictions and ground truth
            visualize_batch(input1, labels, input2, pred, epoch, experiment_dir=experiment_dir, test_losses=epoch_loss, train_test='testing')
