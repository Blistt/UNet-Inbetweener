    ...
    for epoch in range(n_epochs):
        ...
        if epoch % display_step == 0:
            ...
            if my_dataset is not None:
                # Free up unused memory before starting testing process
                torch.cuda.empty_cache()
                unused_loss = test(my_test_dataset, model, criterion, epoch, batch_size=batch_size, device=device, experiment_dir=experiment_dir,
                                    display_step=display_step, train_test='extra_testing')
            ...
            visualize_batch(input1, labels, input2, pred, epoch, experiment_dir=experiment_dir, train_losses=tr_losses, 
                            test_losses=test_losses, train_test='training')
